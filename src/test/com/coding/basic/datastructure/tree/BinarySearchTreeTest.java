...
public class BinarySearchTreeTest {
    ...
    public void testIsValid() {
        Assert.assertTrue(tree.isValidBST());
        BinaryTreeNode<Integer> root = new BinaryTreeNode<>(6);
        root.setLeft(new BinaryTreeNode<>(2));
        root.setRight(new BinaryTreeNode<>(8));
        ...
        root.getLeft().getRight().setLeft(new BinaryTreeNode<>(3));
        tree = new BinarySearchTree<>(root);
        Assert.assertFalse(tree.isValidBST());
    }
    ...
    public void testChildrenPath() {
        List<List<Integer>> result = tree.getLeavesPath();
        Assert.assertEquals("[[6, 2, 1], [6, 2, 4, 3], [6, 2, 4, 5], [6, 8]]", result.toString());
    }

    @Test
    public void testGetLeaves() {
        List<Integer> leaves = tree.getLeaves();
        Assert.assertEquals("[1, 3, 5, 8]", leaves.toString());
    }
}
